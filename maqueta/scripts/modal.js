...
for (categoria in convertir) {
  for (tipo in convertir[categoria]) {
    convertir[categoria][tipo].forEach(element => {
      ...
      if (element.producto_id === 1) {
        element.colores.map(l => l)
...
cerrarModal.addEventListener('click', () => {
    inforProducto.classList.add('modalcerrado');/* primero crea la clase dento del contenedor llamado infoProducto */
    setTimeout( () => { /* esta funcion se usa para que se ejecute en determinado tiempo que le decimos en este caso seran 300 milisegundos  */
        inforProducto.style.display = "none"; 
        inforProducto.classList.remove('modalcerrado');
    }, 300);
});